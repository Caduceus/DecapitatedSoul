...
if ($render_page) {
	...
	<div id="myaccount">
		<h1>My Account</h1>
		<p><h2>Welcome to your account page, <?php echo $user_data['name']; ?>.</h2><br>
			VIP days remaining: <font color="red"><?php echo $user_data['vipdays']; ?></font color="red"></p>
			Current store coins: <font color="red"><?php echo $user_data['coins']; ?></font color="red"></p>
		<?php
		...
		if ($char_array) {
			...
			<table id="myaccountTable" class="table table-striped table-hover">
				...
				foreach ($char_array as $value) {
					...
					echo '<tr>';
					echo '<td><b><i><a href="characterprofile.php?name='. $value['name'] .'">'. $value['name'] .'</a></i></b></td><td>'. $value['level'] .'</td><td>'. $value['vocation'] .'</td><td>'. $value['town_id'] .'</td><td>'. $value['lastlogin'] .'</td><td>'. $value['online'] .'</td><td>'. hide_char_to_name(user_character_hide($value['name'])) .'</td>';
					echo '</tr>';
