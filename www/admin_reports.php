...
// Fetch data from SQL
$reportsData = mysql_select_multi('SELECT id, name, posx, posy, posz, report_description, date, status FROM player_reports ORDER BY id DESC;');
...
if (!empty($_POST)) {
    ...
    // Update SQL
    mysql_update("UPDATE `player_reports` SET `status`='$status' WHERE `id`='$reportId' LIMIT 1;");
    ...
    if ($price > 0) {
        ...
        if ($account !== false) {
            // transaction log
            mysql_insert("INSERT INTO `znote_report_reward` VALUES ('', '$reportId', 'report@admin.".$user_data['name']." to ".$account['email']."', '".$account['id']."', '0', '".$price."')");
            // Process payment
            $data = mysql_select_single("SELECT `coins` AS `old_coins` FROM `accounts` WHERE `id`='".$account['id']."';");
            // Give points to user
            $new_points = $data['old_coins'] + $price;
            mysql_update("UPDATE `accounts` SET `coins`='$new_points' WHERE `id`='".$account['id']."'");

            // Remind GM that he sent points to character
            echo "<font color='green' size='5'>".$playerName." has been granted ".$price." coins for their report.</font>";
        }
    }
