...
?>
<div class="panel">
<div class="page-header"><center><h1>insomaniacs</h1></center></div>
	<?php
	...
	if ($days) {
		...
		$znotePlayers = mysql_select_multi('SELECT `a`.`id`, `b`.`player_id`, `a`.`name`, `a`.`vocation`, `a`.`level`, `a`.`group_id`, `a`.`onlinetime`, `b`.`onlinetimetoday`, `b`.`onlinetime1`, `b`.`onlinetime2`, `b`.`onlinetime3`, `b`.`onlinetime4`, `b`.`onlinetime5`, `b`.`onlinetime6`, `b`.`onlinetime7`,   (`a`.`onlinetime` - `b`.`onlinetimetoday`)  AS `onlinediff` FROM `players` `a` JOIN `znote_players` `b` ON `a`.`id` = `b`.`player_id`  WHERE `a`.`group_id` < 4 ORDER BY `onlinediff` DESC LIMIT '.$limit);
	}
	$limit = $config['powergamers']['limit'];
	...
	elseif(empty($days) && !empty($vocation)) {
		$znotePlayers = mysql_select_multi('SELECT `a`.`id`, `b`.`player_id`, `a`.`name`, `a`.`vocation`, `a`.`level`, `a`.`group_id`, `a`.`onlinetime`, `b`.`onlinetimetoday`, `b`.`onlinetime1`, `b`.`onlinetime2`, `b`.`onlinetime3`, `b`.`onlinetime4`, `b`.`onlinetime5`, `b`.`onlinetime6`, `b`.`onlinetime7`,   (`a`.`onlinetime` - `b`.`onlinetimetoday`)  AS `onlinediff` FROM `players` `a` JOIN `znote_players` `b` ON `a`.`id` = `b`.`player_id`  WHERE `a`.`group_id` < 4 AND `a`.`vocation`='. (int)$vocation .' OR `a`.`vocation`='. ((int)$vocation +4) .' ORDER BY `onlinediff` DESC LIMIT '.$limit);
	}elseif(!empty($days) && empty($vocation)) 
	...
	if($znotePlayers) {
		foreach($znotePlayers as $player)
		{
			...
			echo ($days == 8) ? '<td><center>'. number_format($player['onlinetime7']) .'</center></td>' : '';
			echo ($days == 7) ? '<td><center>'. $player['onlinetime7'] .'</center></td>' : '';
			echo ($days == 6) ? '<td><center>'. $player['onlinetime6'] .'</center></td>' : '';
			echo ($days == 5) ? '<td><center>'. $player['onlinetime5'] .'</center></td>' : '';
			echo ($days == 4) ? '<td><center>'. $player['onlinetime4'] .'</center></td>' : '';
			echo ($days == 3) ? '<td><center>'. $player['onlinetime3'] .'</center></td>' : '';
			echo ($days == 2) ? '<td><center>'. $player['onlinetime2'] .'</center></td>' : '';
			echo ($days == 1) ? '<td><center>'. $player['onlinetime1'] .'</center></td>' : '';
			echo ($today == true) ? '<td><center>'. ($player['onlinetime']-$player['onlinetimetoday']) .'</center></td>' : '';
			echo '</tr>';
		}
	}
	?>
