...
if (empty($_POST) === false) {
	...
	if (empty($_POST['points_char']) === false && empty($_POST['points_value']) === false) {
		...
		$znote_account = mysql_select_single("SELECT `id`, `coins` FROM `accounts` WHERE `id`='". $account['id'] ."';");
		data_dump($znote_account, false, "Fetching existing coins from accounts");

		data_dump(
			array(
				'Old:' => $znote_account['coins'], 
				'New:' => $points, 
				'Total:' => ($znote_account['coins'] + $points)
				),
			false,
			"Points calculation:");
		$points += $znote_account['coins'];
		mysql_update("UPDATE `accounts` SET `coins`='$points' WHERE `id`='". $account['id'] ."';");
	}
...
<ul>
	...
	<li>
		<b>Give ingame coins to character:</b>
		<form action="" method="post">
			<input type="text" name="points_char" placeholder="Character name">
			<input type="text" name="points_value" placeholder="Coins">
			<input type="submit" value="Give coins">
		</form>
	</li>
	<li>
		<b>Teleport Player</b>
