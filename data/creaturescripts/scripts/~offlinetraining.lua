function onLogin(player)
	...
	if isInArray({SKILL_CLUB, SKILL_SWORD, SKILL_AXE, SKILL_DISTANCE}, offlineTrainingSkill) then
		local modifier = topVocation:getAttackSpeed() / 200
		updateSkills = player:addOfflineTrainingTries(offlineTrainingSkill, (configManager.getNumber("rateSkill") * (trainingTime / modifier) / (offlineTrainingSkill == SKILL_DISTANCE and 4 or 2)))
	elseif offlineTrainingSkill == SKILL_MAGLEVEL then
		...
		updateSkills = player:addOfflineTrainingTries(SKILL_MAGLEVEL, (configManager.getNumber("rateMagic") * trainingTime * (vocation:getManaGainAmount() / gainTicks))) --
	end

	if updateSkills then
		player:addOfflineTrainingTries(SKILL_SHIELD, (configManager.getNumber("rateSkill") * trainingTime / 4)) --(configManager.getNumber("rateSkill") * trainingTime / 4) 
	end
