...
local function getMoneyAmount(position)
    ...
    for _, item in ipairs(Tile(position):getItems()) do
        ...
        elseif itemId == ITEM_GOLD_BAR then
            moneyAmount = moneyAmount + item.type * 1000000
        elseif itemId == ITEM_SCARAB_COIN then
            moneyAmount = moneyAmount + item.type * 100000000
        end
    end
...
local function handleMoney(cid, position, bonus)
    ...
    for _, item in ipairs(Tile(position):getItems()) do
        if isInArray({ITEM_GOLD_COIN, ITEM_PLATINUM_COIN, ITEM_CRYSTAL_COIN, ITEM_GOLD_BAR, ITEM_SCARAB_COIN, ITEM_BLOCKING}, item:getId()) then
            item:remove()
        end
    end
    ...
    local scarabCoinAmount = math.floor(goldCoinAmount / 100000000)
    goldCoinAmount = goldCoinAmount - scarabCoinAmount * 100000000
    local goldBarAmount = math.floor(goldCoinAmount / 1000000)
    goldCoinAmount = goldCoinAmount - goldBarAmount * 1000000
    ...
    if goldBarAmount ~= 0 then
        Game.createItem(ITEM_GOLD_BAR, goldBarAmount, position)
    end
    
    if scarabCoinAmount ~= 0 then
        Game.createItem(ITEM_SCARAB_COIN, scarabCoinAmount, position)
    end
...
local function creatureSayCallback(cid, type, msg)
    ...
    if not topCreature then
        npc:say("Please stand here.", TALKTYPE_SAY)
...
function onThink()
    ...
    for _, item in ipairs(Tile(moneyPosition):getItems()) do
        local itemId = item:getId()
        if not isInArray({ITEM_GOLD_COIN, ITEM_PLATINUM_COIN, ITEM_CRYSTAL_COIN, ITEM_GOLD_BAR, ITEM_SCARAB_COIN, ITEM_BLOCKING}, itemId) and ItemType(itemId):isMovable() then
            item:remove()
        end
    end
