...
function onUse(player, item, fromPosition, target, toPosition, isHotkey)
	...
	elseif itemId == strongHealthPot then
		...
		if not doTargetCombatHealth(0, target, COMBAT_HEALING, 450, 700, CONST_ME_MAGIC_BLUE) then
			return false
		end
	...
	elseif itemId == greatSpiritPot then
		if (not isInArray({3, 7}, target:getVocation():getId()) or target:getLevel() < 175) and not getPlayerFlagValue(player, PlayerFlag_IgnoreSpellCheck) then
			player:say("This potion can only be consumed by paladins of level 175 or higher.", TALKTYPE_MONSTER_SAY)
			return true
		end

		if not doTargetCombatHealth(0, target, COMBAT_HEALING, 1200, 2000, CONST_ME_HOLYDAMAGE) or not doTargetCombatMana(0, target, 1200, 2000, CONST_ME_YELLOWSMOKE) then
			return false
		end
		...
		elseif itemId == MasterSpiritPot then
		if (not isInArray({3, 7, 13}, target:getVocation():getId()) or target:getLevel() < 300) and not getPlayerFlagValue(player, PlayerFlag_IgnoreSpellCheck) then
			player:say("This potion can only be consumed by paladins of level 300 or higher.", TALKTYPE_MONSTER_SAY)
			return true
		end

		if not doTargetCombatHealth(0, target, COMBAT_HEALING, 2000, 3500, CONST_ME_HOLYDAMAGE) or not doTargetCombatMana(0, target, 2000, 2500, CONST_ME_YELLOWSMOKE) then
			return false
		end
	...
	elseif itemId == greatHealthPot then
		...
		if not doTargetCombatHealth(0, target, COMBAT_HEALING, 700, 1100, CONST_ME_REDSMOKE) then 
			return false
		end
	...
	elseif itemId == greatManaPot then
		...
		if not doTargetCombatMana(0, target, 1400, 2500, CONST_ME_MAGIC_BLUE) then
			return false
		end
		player:addCondition(exhaust)
		--target:say("Aaaah...", TALKTYPE_MONSTER_SAY)
	...
	elseif itemId == ultimateHealthPot then
		if (not isInArray({4, 8, 9, 10, 14, 15}, target:getVocation():getId()) or target:getLevel() < 200) and not getPlayerFlagValue(player, PlayerFlag_IgnoreSpellCheck) then
			player:say("This potion can only be consumed by knights of level 200 or higher.", TALKTYPE_MONSTER_SAY)
			return true
		end

		if not doTargetCombatHealth(0, target, COMBAT_HEALING, 1400, 2500, CONST_ME_MAGIC_BLUE) then
			return false
		end
		...
		item:remove(1)
		--player:addItem(greatEmptyPot, 1)
	elseif itemId == MasterManaPot then
		if (not isInArray({1,2,5,6,11,12}, target:getVocation():getId()) or target:getLevel() < 300) and not getPlayerFlagValue(player, PlayerFlag_IgnoreSpellCheck) then
			player:say("This potion can only be consumed by sorcerers and druids of level 300 or higher.", TALKTYPE_MONSTER_SAY)
			return true
		end

		if not doTargetCombatMana(0, target, 2200, 4000, CONST_ME_PURPLESMOKE) then
			return false
		end
		player:addCondition(exhaust)
		--target:say("Aaaah...", TALKTYPE_MONSTER_SAY)
		item:remove(1)
		--player:addItem(greatEmptyPot, 1)
	elseif itemId == MasterHealthPot then
		if (not isInArray({4, 8, 9, 10, 14, 15}, target:getVocation():getId()) or target:getLevel() < 300) and not getPlayerFlagValue(player, PlayerFlag_IgnoreSpellCheck) then
			player:say("This potion can only be consumed by knights of level 300 or higher.", TALKTYPE_MONSTER_SAY)
			return true
		end

		if not doTargetCombatHealth(0, target, COMBAT_HEALING, 2200, 4000, CONST_ME_REDSMOKE) then
			return false
		end
